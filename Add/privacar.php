...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $db->prepare("INSERT INTO pcars (registration_number, department, make, model, color, year, owner) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssss", $registration_number, $department, $make, $model, $color, $year, $owner);

    // Execute statement
    if ($stmt->execute()) {
        $_SESSION['success'] = "Private car added successfully!";
    } else {
    ...
    header("Location: addpcar.php"); // Changed to .php extension
    exit();
}
